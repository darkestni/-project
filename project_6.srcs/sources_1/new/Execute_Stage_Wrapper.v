module Execute_Stage_Wrapper (
    ...
    localparam BRANCH_BEQ = 3'b000; // BEQ指令的 funct3 值
    // 内部信号线
    ...
    always @(*) begin
        if (branch_ctrl_from_idex && alu_branch_flag && funct3_from_idex == BRANCH_BEQ
        ...
        else if (branch_ctrl_from_idex && funct3_from_idex == BRANCH_BNE && !alu_branch_flag) begin
            //BNE
            branch_or_jump_to_if = 1'b1;
            target_pc_ex            = pc_from_idex + imm32_from_idex;
        end
    ...
    Controller_EX_Logic u_controller_ex (
        ...
        .alu_result_from_ex(alu_result_addr_to_exmem), // ALU计算出的地址高位

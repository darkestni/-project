module DataHazardDetect (
    ...
    input        idex_isLoad,    // Is the instruction currently in EX a Load?
    input [4:0]  idex_rd_addr,   // Destination register of the instruction in EX
    ...
    // Your connections: .ifid_rs1_addr(rs1_addr_from_id), .ifid_rs2_addr(rs2_addr_from_id)
    ...
    input [4:0]  ifid_rs1_addr,  // rs1 of the instruction currently in ID
    input [4:0]  ifid_rs2_addr,  // rs2 of the instruction currently in ID
    ...
    always @(*) begin
        ...
        if (idex_isLoad && (idex_rd_addr != 5'd0) && // If instr in EX is a Load and its dest is not x0
            ( (idex_rd_addr == ifid_rs1_addr) ||   // And its dest is rs1 of instr in ID
              (idex_rd_addr == ifid_rs2_addr) )    // Or its dest is rs2 of instr in ID
           )
        begin
